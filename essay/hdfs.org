...
*NOTE(dirlt):名词是自己定义的方便理解*
   - NNs balance iteration # 对于hdfs federation来说可能存在多个NN. 并且对于这些NN需要发起多轮balance迭代，每轮迭代称为NNs balance iteration.
   - NN balance iteration # NNs balance iteration内部针对每个NN集群发起一轮balance迭代称为NN balance iteration. 内部会拆解成为多个source发起balance.
   - Source balance iteration # 每个source发起balance, 内部也会多次挑选block来做move. 这样每次move称为source balance iteration.
...
*restriction*
   - timeout = 20min
   - block size = 2 * scheduledSize # scheduledSize在NN balance iteration的chooseNodes阶段设置
...
#+BEGIN_SRC Java
    ...
    // 从srcBlockList里面过滤一些在一段时间内已经移动过的block. 实现可以参考MovedBlocks.
...
#+BEGIN_SRC Java    
    // 是否应该尝试获取更多的blocks，srcBlockList大小<5并且还有余量来获取blocks.
