...
int Message::read(int fd) {
  ...
  while(1) {
    ...
    if(body_len >= body.size()) {
      SPERM_DEBUG("(sizeof(body_len) + body_len)=%zu, body_off=%zu",
...
int AsyncSocket::bindAndListen(const char* ip, int port, int backlog) {
  ...
  if(fd == -1) {
    SPERM_WARNING("create_tcp_socket failed");
    return -1;
  }
  set_tcp_nodelay(fd);
  set_ip_reuseaddr(fd);
  ...
  if(tcp_bind_listen(fd, ip, port, backlog) == -1) {
    SPERM_WARNING("tcp_bind_listen failed");
    return -1;
  }
  initNetReadEvent(AA_BIND_LISTEN, fd);
  if(itachi->launch(this) == -1) {
    SPERM_WARNING("itachi->launch(%p) failed", this);
    return -1;
  }
  return 0;
}
...
int AsyncSocket::connect(const char* ip, int port, int timeout_ms) {
  ...
  if(code == -1) {
    SPERM_WARNING("tcp_connect failed");
    return -1;
  }
...
int AsyncSocket::recv(Message* msg, int timeout_ms) {
  ...
  if(msg_) {
    SPERM_WARNING("message resides in socket");
    return -1;
  }
  msg->onRecv();
...
void AsyncSocket::Client::onComplete(AsyncContext* ctx_) {
  ...
  switch(ctx->action) {
    ...
    case AA_READ:
      ...
      if(code == Message::kDone) {
        ...
        if(itachi->launch(ctx) == -1) {
          SPERM_FATAL("itachi->launch(%p) failed", ctx);
        }
    ...
    case AA_WRITE:
      ...
      break;
    default:
      SPERM_FATAL("unknown ctx(%p)->action(%d)", ctx, ctx->action);
  }
  return ;
}
